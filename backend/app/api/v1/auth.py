...
from app.models.user import User, UserToken, Company, Role
...
async def register(payload: RegisterRequest, db: AsyncSession = Depends(get_db)):
    """Create a new recruiter account. Optionally creates the company (tenant)."""

    result = await db.execute(select(User).where(User.email == payload.email, User.deleted == False))  # noqa: E712
    ...
    user = User(
        ...
        is_active=True,      # simplify for now; add email verification later
        is_verified=True,
    )
    db.add(user)
    await db.commit()
...
async def login(payload: LoginRequest, db: AsyncSession = Depends(get_db)):
    """Authenticate user and return access + refresh tokens with role/permissions."""

    result = await db.execute(
        select(User)
        .options(selectinload(User.role).selectinload(Role.permissions))
        .where(User.email == payload.email, User.deleted == False)  # noqa: E712
    )
    ...
    if user.role:
        ...
        permissions_list = [p.code_name for p in user.role.permissions]
...
async def refresh_token(payload: RefreshRequest, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(User).where(User.id == user_id, User.deleted == False))  # noqa: E712
    user = result.scalar_one_or_none()
...
async def forgot_password(payload: ForgotPasswordRequest, db: AsyncSession = Depends(get_db)):
    ...
    # TODO (Phase 14): send via Celery email task instead of returning it here.
    return success_response(
        {
            "message": "Reset code sent to your email.",
            "code": code if settings.DEBUG else None,  # DEBUG-only convenience for Postman testing
        }
    )
...
async def verify_otp(payload: VerifyOTPRequest, db: AsyncSession = Depends(get_db)):
    ...
    if datetime.now(timezone.utc) - user.password_reset_code_created_at.replace(
        tzinfo=timezone.utc
    ) > timedelta(minutes=3):
    ...
    return success_response({"message": "OTP verified.", "reset_token": reset_token})
...
async def reset_password(payload: ResetPasswordRequest, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(User).where(User.password_link_token == payload.reset_token, User.deleted == False)  # noqa: E712
    )
    user = result.scalar_one_or_none()
