...
app/api/v1/users.py
────────────────────
Users, Roles, Permissions, Companies endpoints.

GET    /api/v1/users/me               → current user profile
PATCH  /api/v1/users/me               → update profile
GET    /api/v1/users/                 → list users (superuser)
GET    /api/v1/users/{id}             → user detail (superuser)
PATCH  /api/v1/users/{id}/block       → block/unblock user (superuser)

GET    /api/v1/users/roles/           → list roles
POST   /api/v1/users/roles/           → create role (superuser)
PATCH  /api/v1/users/roles/{id}       → update role (superuser)
DELETE /api/v1/users/roles/{id}       → delete role (superuser)

GET    /api/v1/users/permissions/     → list all permissions

GET    /api/v1/users/companies/       → list companies (superuser)
POST   /api/v1/users/companies/       → create company (superuser)
PATCH  /api/v1/users/companies/{id}   → update company (superuser)
DELETE /api/v1/users/companies/{id}   → soft delete company (superuser)
"""
...
from app.schemas.user import (
    UserUpdate, RoleCreate, RoleUpdate, CompanyCreate, CompanyUpdate,
)
            ...
            "role": (
                {
                    ...
                    "code_name": current_user.role.code_name,
                }
...
# ─────────────────────────────────────────────────────────────────
#  User List & Detail  (superuser only)
# ─────────────────────────────────────────────────────────────────
...
async def list_users(
    ...
    if admin.company_id:
        users, total = await repo.list_by_company(str(admin.company_id), page, page_size)
    else:
        users, total = await _list_all_users(db, page, page_size)
...
async def _list_all_users(db, page, page_size):
    ...
    q = select(UserModel).where(UserModel.deleted == False).order_by(UserModel.created_at.desc())  # noqa: E712
    ...
    data = [
        {
            ...
            "permissions": [{"id": str(p.id), "name": p.name, "code_name": p.code_name} for p in r.permissions],
        }
        for r in roles
    ]
...
async def create_role(payload: RoleCreate, admin: User = Depends(require_superuser), db: AsyncSession = Depends(get_db)):
    ...
    await db.commit()
    return success_response({"id": str(role.id), "name": role.name, "code_name": role.code_name}, status_code=201)
